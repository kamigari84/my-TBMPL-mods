...
static class PrefabPatcher {
  ...
  public static void ReplaceComponent<TSource, TTarget>(GameObject prefab, Func<GameObject, bool> checkDeps)
    ...
    prefab.AddComponent<TTarget>();
        ...
        public static void RemoveComponent<TSource>(GameObject prefab, Func<GameObject, bool> checkDeps)
    where TSource : BaseComponent
        {
            if ( prefab.GetComponent<TSource>() == null || !checkDeps(prefab))
            {
                return;
            }
            DebugEx.Fine("Remove component on prefab: name={0}, removedComponent={1}",
                         prefab.name, typeof(TSource));
            Object.DestroyImmediate(prefab.GetComponent<TSource>());
        }
